...
class OpenCVConan(ConanFile):
    ...
    def requirements(self):
        ...
        if self.options.with_gstreamer:
            ...
            self.requires.add('gst-plugins-base/1.19.2')
    ...
    def validate(self):
        ...
        if self.options.with_inf_engine and os.environ.get('INTEL_OPENVINO_DIR') is None:
    ...
    def _configure_cmake(self):
        ...
        self._cmake.definitions['BUILD_opencv_world'] = self.options.world
        ...
        self._cmake.definitions["WITH_INF_ENGINE"] = self.options.with_inf_engine
        if self.options.with_inf_engine:
    ...
    def _opencv_components(self):
        ...
        if self.options.world:
            opencv_components = [{"target": "opencv_world", "lib": "world", "requires": ["zlib::zlib"] + parallel() + eigen() + ipp() + imageformats_deps() + freetype() + gtk() + quirc() + ffmpeg() + protobuf() + gstreamer() + gstpluginbase()}]
        else:
            opencv_components = [
                ...
                {"target": "opencv_videoio",    "lib": "videoio",    "requires": (
                    ...
                    + eigen() + ffmpeg() + ipp() + gstreamer() + gstpluginbase())},
