...
class OpenCVConan(ConanFile):
    ...
    def requirements(self):
        ...
        if self.options.get_safe("with_ffmpeg"):
            # opencv doesn't support ffmpeg >= 5.0.0 for the moment (until 4.6.0 at least)
    ...
    def generate(self):
        ...
        if is_msvc(self):
        	if not is_msvc_static_runtime(self):
	            self.output.info("Dynamic runtime, opencv build will use flag /MD (or /MDd in debug mode)")
	            tc.variables["CMAKE_CXX_FLAGS_RELEASE"] = "/MD"
	            tc.variables["CMAKE_CXX_FLAGS_DEBUG"] = "/MDd"
	            tc.variables["CMAKE_CXX_FLAGS_MINSIZEREL"] = "/MD; -Os"
	            tc.variables["CMAKE_CXX_FLAGS_RELWITHDEBINFO"] = "/MD"
	        else:
	            self.output.info("Static runtime, opencv build will use flag /MT (or /MTd in debug mode)")
        ...
        if self.options.with_cuda:
            ...
            # set NVCC runtime according to the runtime used for building opencv.
            if is_msvc(self):
                if not is_msvc_static_runtime(self):
