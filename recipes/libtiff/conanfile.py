...
class LibtiffConan(ConanFile):
    ...
    url = "https://github.com/Solar-Framework/conan-solar/recipes/libtiff/"
    license = "MIT"
    ...
    def requirements(self):
        ...
        if self.options.jpeg:
            self.requires("libjpeg/9d@conan-solar/stable")
        if self.options.jbig:
            self.requires("jbig/20160605")
        if self.options.get_safe("zstd"):
            self.requires("zstd/1.4.5")
        if self.options.get_safe("webp"):
            self.requires("libwebp/1.0.3@conan-solar/stable")
