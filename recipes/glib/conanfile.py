...
class GLibConan(ConanFile):
    ...
    url = "https://github.com/Solar-Framework/conan-solar/recipes/glib"
    ...
    requires = "zlib/1.2.11@conan-solar/stable", "libffi/3.2.1"
    ...
    def requirements(self):
        if self.options.with_pcre:
            self.requires("pcre/8.41@conan-solar/stable")
        ...
        if self.settings.os == "Linux":
            ...
            if self.options.with_selinux:
                self.requires("libselinux/2.9@conan-solar/stable")
        else:
            # for Linux, gettext is provided by libc
            self.requires("gettext/0.20.1@conan-solar/stable")
